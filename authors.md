# Authors and Citation

## Authors

- **[Balasubramanian
  Narasimhan](https://statistics.stanford.edu/people/balasubramanian-narasimhan)**.
  Author, maintainer. [](https://orcid.org/0000-0001-5852-7639)

- **SCIP Optimization Suite Authors**. Copyright holder.  
  SCIP, SoPlex, PaPILO libraries

## Citation

Source:
[`inst/CITATION`](https://github.com/bnaras/scip/blob/main/inst/CITATION)

Narasimhan B (2026). *scip: R Interface to the SCIP Optimization Suite*.
R package.

    @Manual{,
      title = {scip: R Interface to the SCIP Optimization Suite},
      author = {Balasubramanian Narasimhan},
      year = {2026},
      note = {R package},
    }

Hojny C, others (2025). “The SCIP Optimization Suite 10.0.” Optimization
Online. Technical Report, <https://doi.org/10.48550/arXiv.2411.14927>.

    @TechReport{,
      title = {The SCIP Optimization Suite 10.0},
      author = {Christopher Hojny and others},
      year = {2025},
      institution = {Optimization Online},
      url = {https://doi.org/10.48550/arXiv.2411.14927},
      note = {Technical Report},
    }

Achterberg T (2009). “SCIP: Solving Constraint Integer Programs.”
*Mathematical Programming Computation*, **1**(1), 1–41.
[doi:10.1007/s12532-008-0001-1](https://doi.org/10.1007/s12532-008-0001-1).

    @Article{,
      title = {SCIP: Solving Constraint Integer Programs},
      author = {Tobias Achterberg},
      journal = {Mathematical Programming Computation},
      year = {2009},
      volume = {1},
      number = {1},
      pages = {1--41},
      doi = {10.1007/s12532-008-0001-1},
    }

Achterberg T, Berthold T, Koch T, Wolter K (2008). “Constraint Integer
Programming: a New Approach to Integrate CP and MIP.” In *Integration of
AI and OR Techniques in Constraint Programming for Combinatorial
Optimization Problems*, volume 5015 series Lecture Notes in Computer
Science, 6–20. Springer.
[doi:10.1007/978-3-540-68155-7_4](https://doi.org/10.1007/978-3-540-68155-7_4).

    @InCollection{,
      title = {Constraint Integer Programming: a New Approach to Integrate CP and MIP},
      author = {Tobias Achterberg and Timo Berthold and Thorsten Koch and Kati Wolter},
      booktitle = {Integration of AI and OR Techniques in Constraint Programming for Combinatorial Optimization Problems},
      series = {Lecture Notes in Computer Science},
      volume = {5015},
      pages = {6--20},
      year = {2008},
      publisher = {Springer},
      doi = {10.1007/978-3-540-68155-7_4},
    }
